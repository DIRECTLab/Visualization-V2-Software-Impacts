...
router.get("/", shared.asyncWrapper(async (req, res) => {
  ...
  if (req.query.time) {
    try {
      const entry = await Speed.findOne({
        where: {
  ...
  const entries = await Speed.findAll({
    where: {
      ViricitibusVid: busId,
      time: {
...
router.post('/', shared.asyncWrapper(async (req, res) => {
  ...
  const { Viricitibus, Speed } = res.locals.models;
  ...
  if (!entry) {
    try {
      const current = await Speed.create({...req.body, ViricitibusVid: busId});
