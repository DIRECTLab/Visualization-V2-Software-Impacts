...
router.get("/", shared.asyncWrapper(async (req, res) => {
  ...
  const entries = await Odo.findAll({
    where: {
      ViricitibusVid: busId,
      time: {
...
router.post('/', shared.asyncWrapper(async (req, res) => {
  ...
  const { Viricitibus, Odo } = res.locals.models;
  ...
  if (!entry) {
    try {
      const current = await Odo.create({...req.body, ViricitibusVid: busId});
