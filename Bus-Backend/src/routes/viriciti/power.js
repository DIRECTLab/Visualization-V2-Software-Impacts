...
router.get("/", shared.asyncWrapper(async (req, res) => {
  ...
  if (req.query.time) {
    try {
      const entry = await Power.findOne({
        where: {
  ...
  const entries = await Power.findAll({
    where: {
      ViricitibusVid: busId,
      time: {
...
router.post('/', shared.asyncWrapper(async (req, res) => {
  ...
  const { Viricitibus, Power } = res.locals.models;
  ...
  if (!entry) {
    try {
      const current = await Power.create({...req.body, ViricitibusVid: busId});
