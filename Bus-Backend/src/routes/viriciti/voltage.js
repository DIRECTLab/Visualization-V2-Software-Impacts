...
router.get("/", shared.asyncWrapper(async (req, res) => {
  const { Voltage } = res.locals.models;
  ...
  if (req.query.time) {
    try {
      const entry = await Voltage.findOne({
        where: {
  ...
  const entries = await Voltage.findAll({
    where: {
      ViricitibusVid: busId,
      time: {
...
router.post('/', shared.asyncWrapper(async (req, res) => {
  ...
  const { Viricitibus, Voltage } = res.locals.models;
  ...
  if (!entry) {
    try {
      const current = await Voltage.create({...req.body, ViricitibusVid: busId});
